...
def print_hosts(hosts, previous_host):

    subprocess.call("clear" if os.name == "posix" else "cls")
    print(
        "\n  __Hostname________ ____IP_address___ ___MAC__address___ __Avail__ __last__heard______\n"
    )
    for host in hosts.values():

        if not host["availability"]:
            color = Fore.RED
        elif host["hostname"] in previous_host and host == previous_host[host["hostname"]]:
            color = Fore.GREEN
        else:
