...
class DnsMonitor(ServiceMonitor):
    ...
    def get_status(self):
        ...
        try:
            ...
            print(
                f"!!! DNS monitor: DNS request timed out  for {self.data}: {e}"
            )
            return False, 0.0
        except BaseException as e:
            print(f"!!! DNS monitor: Eception occured: {e}")
            return False, 0.0

        if (
            response is not None
            and response.response is not None
            and len(response.response.anwser) > 0
        ):
            return True, time.time()-time_start
    ...
    def get_status(self):
        ...
        try:
            ...
            subprocess.check_output(
                ["ping", "-c3", "_n", "-i10.5", "-W2", self.target]
            )
            return True, time.time()-time_start
