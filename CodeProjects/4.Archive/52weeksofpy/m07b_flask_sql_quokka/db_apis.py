...
def get_as_dict(model_obj):
    ...
    return {k: v  for (k,v) in model_obj.__dict__.items() if not k.startwith("_")}
...
def get_all_service():
    ...
    services = {service_objs.name: get_as_dict(service_obj) for service_obj in service_objs}
    return services
