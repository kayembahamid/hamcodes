...
scapy.sniff(iface='enp0s3', prn=print_packet, filter="tcp port http", count=10)
...
# DISCOVER HOST ON THE NETWORK USING ARPING FUNCTION
print("\n\n--- Discovery hosts on network using arping() function --------")
...
while True:
    ...
    # answer, unans = scapy.sr(IP(dst="192.168.254.254")/TCP(flags="S", sport=666, dport=[22, 80, 21, 443]), timeout=1)
    answers, unans = scapy.sr(IP(dst=ip)/TCP(falgs="S", sport=666, dport=(1, 1024)), timeout=10)
