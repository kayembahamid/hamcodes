...
# devices[NXOS_SSH] = copy.deepcopy(device[NXOS])
...
for device_type, device in devices.items():
    ...
    print("\n----- vlans ----------")
    try:
        print(json.dumps(napalm_device.get_interfaces(), sort_key=True, indent=4))
    ...
    print("\n----- environment ----------")
    try:
        print(json.dumps(napalm_device.get_environemt(), sort_keys=True, indent=4))
