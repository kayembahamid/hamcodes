...
commands = {SHOW_IP_ROUTE:{IOS: "show ip route",
            ...
            SHOW_INT_BRIEF: {IOS: "show interfaces brief",
...
for command_type, command in commands.items():
    ...
    # print(f"\n---- ... for IOS: {command[IOS]} ---------------")
    # csr_output = csr_connection.send_command(command[NXOS])
    # print(csr_output)
    #
...
# nxos_connction.disconnect()
xr_connection.disconnect()

# Now the harder part - parsing the output into some 'normalize' formart
...
if xr_version_raw:
    ...
    #       print(f"----> NXOS version parsed from output: {csr_version_match.group(1)}")

    if xr_version_match:
        print(f"----> IOSR version parsed from output: {xr_version_match.group(1)}")
