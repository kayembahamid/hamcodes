...
print(
    "\n\n------- discovery host on network using arping() function -------------"
)
...
rsp = requests.post("htpp://127.0.0.1:5000/hosts", json=hosts)
if rsp.status_code != 204:
    print(
        f"{str(datetime.now())[:-3]}: Error posting to /hosts, response: {rsp.status_code}, {rsp.content}"
    )

response = requests.get("http://127.0.0.1:5000/hosts", json =hosts)
