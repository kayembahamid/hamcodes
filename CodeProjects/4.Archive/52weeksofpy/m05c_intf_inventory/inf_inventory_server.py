...
def inventory():
    ...
    if inventory_file_type.lower() == 'yaml':
        ...
        return "missing or invalid type, shoulkd be yaml, json, csv", 400


@app.route("/interface_counters", mthods=["GET"])
...
def get_intf_counters(device_name, device_type, port, credentrials):
    ...
    device = driver(
        ...
        username=credentrials["username"],
        password=credentrials["password"],
        optional_args={"port:port"},
    )

    device.open()
    counters = device.get_interfaces_counters()
    return "sucsess", json.dumps(counters)
