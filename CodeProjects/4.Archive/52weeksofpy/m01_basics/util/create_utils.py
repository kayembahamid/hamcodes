...
def create_devices(num_devices=1, num_subnets=1, random_ip=False):
    ...
    if num_devices > 254 or num_subnets > 254:
        print("Error: too many device and/or subnets requested")
...
def create_network(num_devices=1, num_subnets=1):
    ...
    # then modify it to become an entire set of subnets, devices, devices, and interfaces.

    devices = create_devices(num_devices, num_subnets)

    # Once again we are just going to consider the subnet to be first three bytes
    ...
    for device in devices:
        ...
        # but for simplicity we are just going to replace the byte with ""0
...
def create_devices_gen(num_devices=1, num_subnets=1):
    ...
    if num_devices > 254 or num_subnets > 254:
        print("Error: too many device any/or subnets requested")
        return None
