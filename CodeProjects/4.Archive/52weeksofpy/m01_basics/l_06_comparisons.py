...
for index, device_a in enumerate(devices):
    for device_b in devices[index+1:]:
        if device_a["name"] == device_b["name"]:
            print(f"Found match! {device_a['name']} fo both {device_a['ip']} and {device_b['ip']}")
...
if devices2 == devices:
    ...
    print("     -----> Moral: Assigment is NOT the same as copy!")
else:
    print("  HUh?")
...
if devices2 == devices:
    ...
    print("    -----> Result: Uh-oh - I just screwwd up the original version!!")
else:
    print("   Huh?")
...
if new_set_of_devices == devices:
    ...
    print("\n   Comparison of the complex, deep data is easy in python")
    print("   ----> Moral: you can compare any two data structure, no matter how deeply nested")
...
d1_different = DeviceWithEq("device 1", "10.10.10.2")
...
if d1 == d1_different:
    print(f"    !!! using __eq__ : uh-oh,  classes equal and they should be")
else:
...
d1_equal = DeviceWithNoEq("device", "10.10.10.1")
d1_same = d1
