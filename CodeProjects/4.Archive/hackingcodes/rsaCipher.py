...
def main():
    # Run a test that encrypts a message to a file or decrypts a message
    # from a file.
...
def getBlocksFromText(message, blockSize=DEFAULT_BLOCK_SIZE):
    # Converts a string message to a list of block integers. Each interger
    ...
    for blockStart in range(0, len(messageBytes), blockSize):
        # Calculate the block integer for this block for text
        blockInt = 0
...
def encryptMessage(message, key, blockSize=DEFAULT_BLOCK_SIZE):
    ...
    for block in getBlocksFromText(message, blockSize):
        # ciphertext = plaintext ˆ e mod n
