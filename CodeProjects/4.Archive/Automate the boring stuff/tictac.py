...
ALL_SPACES = list('123456789') #The keys for a TTT board dictornary.
...
def main():
    ...
    while True:
        print(getBoardStr(gameBoard)) #Dispaly the board on the screen.

        # kep asking for player until they enter a number 1-9:
        move = None
        while not isValidSpace(gameBoard, move):
            print('what is the {}\'s move? (1-9)'.format(currentPlayer))
            move = input()
        updateBoard(gameBoard, move, currentPlayer)  # Make the move.

        # check if the game is over:
        if isWinner(gameBoard, currentPlayer): # Frist check for victory.
...
def getBlankBoard():
    ...
    board = {} # The game is represented as a python dictionary.
    for space in ALL_SPACES:
        board[space] = BLANK # All spaces start as blank.
    return board

def getBoardStr(board):
    """Return atext representation of the board."""
    return '''
    {}|{}|{}  1 2 3
    -+-+-
    {}|{}|{}  4 5 6
...
def isValidSpace(board, space):
    """Return true if the space on board is a valid space number and the space is blank"""
